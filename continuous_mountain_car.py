...
class Continuous_MountainCarEnvSpecial(gym.Env):
    """
    The agent (a car) is started at the bottom of a valley. For any given state
    the agent may choose to accelerate to the left, right or cease any
    acceleration. The code is originally based on [this code](http://incompleteideas.net/MountainCar/MountainCar1.cp)
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(self, goal_velocity=0):
        ...
        self.max_action = 1.0
        self.min_position = -9
        self.max_position = 22
        self.max_speed = 0.07
        self.goal_position = (
            21.4  # was 0.5 in gym, 0.45 in Arnaud de Broissia's version
        )
        self.goal_velocity = goal_velocity
        self.power = 0.0015
    ...
    def _height(self, xs):
        y = 1.3*(1.1* np.sin(0.8*xs - 2.8) + 0.16*xs+3)
        # 修改后的函数
        return y
